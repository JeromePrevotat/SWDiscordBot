...
class KhaBot(commands.Bot):
    ...
    async def get_channel_last_msg(self, ctx):
        """Returns the last message of the Context Channel."""
        arg = await ctx.channel.history(limit=1).flatten()
        msg = arg[0].content
        return msg

    def get_cmd_arg(self, msg):
        """Returns a list of all Arguments passed to the Command, if any.
        This list also includes Options. If no Arguments, returns None."""
        arg = msg.split(' ')
        argList = []
        optList = []
        cmd = arg[0] + ' ' + arg[1]
